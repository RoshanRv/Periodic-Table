    ...
    <section >
        <div className={`lg:w-[81rem] md:w-[45rem] w-[18rem] mx-auto mt-3 pb-4 ${!tab&&'min-h-[88vh]'}`}>
            <div className="flex justify-between">
                ...
                {start==0&&<label className='text-xs md:text-lg md:mx-2'>Difficulty:</label>}
                {start==0&&<select value={difficulty} onChange={(e)=>setDifficulty(e.target.value)} className='bg-transparent outline-0 border-b-2 border-black md:pb-1 text-xs md:text-lg mx-1 md:mx-4 '>
                        <option value={1}>Easy</option>
                        <option value={2}>Medium</option>
                        <option value={3}>Hard</option>
                    </select>}
                    {(start==0&&!replay)&&<button className="md:px-2 md:py-1 px-1 text-[.6rem] md:text-lg bg-gradient-to-tr from-[#d279ff] to-[#96e7ff] rounded-md border-2" onClick={()=>getRandom()}>Play</button>}
                    {(start==0&&replay)&&<button className="md:px-2 md:py-1 px-1 text-[.6rem] md:text-lg bg-gradient-to-tr from-[#d279ff] to-[#96e7ff] rounded-md border-2" onClick={()=>getRandom()}>Play Again</button>}
                    {start==1&&<button className="md:px-2 md:py-1 px-1 text-[.6rem] md:text-lg bg-gradient-to-tr from-[#d279ff] to-[#96e7ff] rounded-md border-2" onClick={()=>stop()}>Quit</button>}
                    <div className='group relative'><FontAwesomeIcon icon={faCircleInfo} className='bg-gray-200 text-[.5rem] md:text-sm rounded-full text-purple-300 ml-1 cursor-pointer border-2'/>
                        <div className='scale-0 group-hover:scale-100 top-6 -left-12 md:-left-40 text-center transition-transform bg-black/80 text-yellow-300 absolute p-1 text-xs w-[35vw]'>
                            <h1>How to Play:</h1>
                            <p>An Element Name is Shown, Incorrect Guesses <span className='text-red-500'>Deducts 50</span> Pts. while Correct Guesses <span className='text-green-500'>Increases 100</span> Pts.</p>
                        </div>
                    </div>
            ...
            {tab&&(<div className={`md:grid md:grid-cols-2 lg:grid-cols-3   hidden `}>
                ...
                <div className='flex items-center'>
                    ...
                    <p className='px-2 text-xs lg:text-md'>Metals</p>
                </div>
                <div className='flex items-center'>
                ...
                <div className='flex items-center'>
                    ...
                    {/*  SCORE    */}
                    {!tab&&(<div className='text-center bg-white h-max rounded-lg mt-1 justify-center'>
                        {start==1&&<h1 className='md:text-lg text-xs font-bold text-blue-600  md:py-2 px-3 py-1 md:px-6'>{rand}</h1>}
                        {(end&&highscore)&&<h1  className='md:px-5 px-2 md:py-2 py-[0.15rem] text-[.65rem] md:text-lg'>Highscore: <span className={` font-bold ${highscore>0?'text-green-500':'text-red-600'}`}>{highscore}</span></h1>}
                
                        {(replay||start==1)&&<h1 className='bg-black/10 md:py-1 md:px-5 px-2 text-[.65rem] md:text-lg'>Score: <span className={` font-bold ${score>0?'text-green-500':'text-red-600'}`}>{score}</span></h1>}
                        </div>)}
